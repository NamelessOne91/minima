...
// Updated Commit function
func (s *FileStorage) Commit() error {
	oldDir := s.directory + "-old"
	tmpDir := s.directory + "-in-progress"

	// If in-progress contains actual packages, it is a "Clean Swap" candidate.
	// Otherwise, it's a "Split" situation where we only have metadata in -in-progress.
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() {
			...
			if _, check := PackageExtensions[ext]; check {
				found = true
				return filepath.SkipDir // Stop walking as soon as one is found
			}
		}
		return nil
	})
